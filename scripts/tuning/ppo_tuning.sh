#!/bin/bash
# PPO 하이퍼파라미터 튜닝 스크립트
# 실행: chmod +x scripts/tuning/ppo_tuning.sh && bash scripts/tuning/ppo_tuning.sh [environment]
...
ENVIRONMENT=$1
echo "=========================================="
echo "PPO 하이퍼파라미터 튜닝 시작 (환경: $ENVIRONMENT)"
echo "시작 시간: $(date)"
echo "=========================================="
...
# 기본 config 파일 확인
BASE_CONFIG="config/${ENVIRONMENT}/ppo.yaml"
...
TUNING_COUNT=0
TOTAL_EXPERIMENTS=$((${#learning_rates[@]} * ${#clip_ratios[@]} * ${#gae_lambdas[@]}))

echo "총 ${TOTAL_EXPERIMENTS}개의 하이퍼파라미터 조합을 테스트합니다."
echo

for lr in "${learning_rates[@]}"; do
    for clip in "${clip_ratios[@]}"; do
        for gae in "${gae_lambdas[@]}"; do
            TUNING_COUNT=$((TUNING_COUNT + 1))
            
            # 실험 이름 생성
            EXP_NAME="${ENVIRONMENT}_ppo_tune_lr${lr}_clip${clip}_gae${gae}"
            
            echo "=========================================="
            echo "실험 ${TUNING_COUNT}/${TOTAL_EXPERIMENTS}: ${EXP_NAME}"
            echo "lr_actor=${lr}, clip_ratio=${clip}, gae_lambda=${gae}"
            echo "=========================================="
            ...
            sed -i "s/clip_ratio: .*/clip_ratio: ${clip}/" "$TEMP_CONFIG"
            sed -i "s/gae_lambda: .*/gae_lambda: ${gae}/" "$TEMP_CONFIG"

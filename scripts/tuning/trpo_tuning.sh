#!/bin/bash
# TRPO 하이퍼파라미터 튜닝 스크립트
# 실행: chmod +x scripts/tuning/trpo_tuning.sh && bash scripts/tuning/trpo_tuning.sh [environment]
...
ENVIRONMENT=$1
echo "=========================================="
echo "TRPO 하이퍼파라미터 튜닝 시작 (환경: $ENVIRONMENT)"
echo "시작 시간: $(date)"
echo "=========================================="
...
# 기본 config 파일 확인
BASE_CONFIG="config/${ENVIRONMENT}/trpo.yaml"
...
max_kls=(0.005 0.01 0.02)
dampings=(0.05 0.1 0.2)

TUNING_COUNT=0
TOTAL_EXPERIMENTS=$((${#learning_rates[@]} * ${#max_kls[@]} * ${#dampings[@]}))

echo "총 ${TOTAL_EXPERIMENTS}개의 하이퍼파라미터 조합을 테스트합니다."
echo

for lr in "${learning_rates[@]}"; do
    for kl in "${max_kls[@]}"; do
        for damp in "${dampings[@]}"; do
            TUNING_COUNT=$((TUNING_COUNT + 1))
            
            # 실험 이름 생성
            EXP_NAME="${ENVIRONMENT}_trpo_tune_lr${lr}_kl${kl}_damp${damp}"
            
            echo "=========================================="
            echo "실험 ${TUNING_COUNT}/${TOTAL_EXPERIMENTS}: ${EXP_NAME}"
            echo "lr=${lr}, max_kl=${kl}, damping=${damp}"
            echo "=========================================="
            ...
            sed -i "s/max_kl: .*/max_kl: ${kl}/" "$TEMP_CONFIG"
            sed -i "s/damping: .*/damping: ${damp}/" "$TEMP_CONFIG"

#!/bin/bash
# SQL 하이퍼파라미터 튜닝 스크립트
# 실행: chmod +x scripts/tuning/sql_tuning.sh && bash scripts/tuning/sql_tuning.sh [environment]
...
ENVIRONMENT=$1
echo "=========================================="
echo "SQL 하이퍼파라미터 튜닝 시작 (환경: $ENVIRONMENT)"
echo "시작 시간: $(date)"
echo "=========================================="
...
# 기본 config 파일 확인
BASE_CONFIG="config/${ENVIRONMENT}/sql.yaml"
...
TUNING_COUNT=0
TOTAL_EXPERIMENTS=$((${#learning_rates[@]} * ${#temperatures[@]} * ${#batch_sizes[@]}))
...
for lr in "${learning_rates[@]}"; do
    for temp in "${temperatures[@]}"; do
        for batch in "${batch_sizes[@]}"; do
            ...
            # 실험 이름 생성
            EXP_NAME="${ENVIRONMENT}_sql_tune_lr${lr}_temp${temp}_batch${batch}"
            
            echo "=========================================="
            echo "실험 ${TUNING_COUNT}/${TOTAL_EXPERIMENTS}: ${EXP_NAME}"
            echo "lr_actor=${lr}, temperature=${temp}, batch_size=${batch}"
            echo "=========================================="
            ...
            sed -i "s/temperature: .*/temperature: ${temp}/" "$TEMP_CONFIG"
